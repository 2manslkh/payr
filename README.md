...
The repository baseline on `main` is now `v1.4.1` at `923ef3f` ([PR #14](https://github.com/2manslkh/payr/pull/14) merged). This is distinct from the recorded R09 deployment evidence: R09 was released as `v1.3.0` at `2dfa8db159c19f95a5af9e99cb4a5a428b2c2e2a` ([PR #12](https://github.com/2manslkh/payr/pull/12)). Protected invoices/PDFs, Arc native-USDC payments, reconciliation, immutable receipts, durable delivery workers, and the four-tool Claude MCP endpoint are implemented. The R09 record verifies its tag, post-merge CI, and production's identical reviewed release-head tree; it does not establish deployment of `v1.4.1` or the current follow-up. See [R09 release evidence](docs/ops/r09-release.md).

The release has explicit live limits: Claude connector and full external-wallet rehearsals remain unverified. One approved R08 receipt email was provider-verified as delivered, but human inbox opening and unattended delivery are not claimed; production receipt email remains disabled. [STATUS.md](STATUS.md) tracks the remaining proof/submission work. Root `integration/root-updates` retains unique discovery/roadmap commits and uncommitted dashboard/install/docs work being integrated through approved feature-group commits and one combined PR into `main`, not already released or deployed. The [root cleanup inventory](docs/ops/repository-cleanup.md) is a historical checkpoint with a current integration note, not a current branch inventory.
