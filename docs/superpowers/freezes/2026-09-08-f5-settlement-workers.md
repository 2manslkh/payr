...
- `src/lib/email/outbox-contracts.ts`, `outbox.ts`, `resend.ts`, `src/lib/db/outbox.ts`, and migration `008`: ready-receipt dependency, one deduplicated logical recipient with retained roles, stable payload hash/key, durable request marker and immutable first-attempt time, deterministic retry timing, and manual review at the 24-hour ambiguity boundary.
