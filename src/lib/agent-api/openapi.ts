...
const accountAuth = "X-Payr-Service-Key is always required. Also supply exactly one account credential: Authorization: Bearer <pac_UUID.secret> OR the top-level accountCredential body string, never both. The service-key-only OpenAPI security alternative is valid ONLY when accountCredential is supplied in the body; it is not account-free access. Workspace identity and scopes come from the account credential, not input. Bazantic does not forward Authorization upstream; use the body fallback there. Generated gateway tools wrap it as requestBody.accountCredential beside requestBody.input; use only fields present in discovered schemas. Supply the raw credential without a Bearer prefix on every protected call. Verify with get_account before business actions. MCP settings do not automatically inject body fields; OAuth login is not this flow.";
const secretHandling = "Keep service keys out of prompts, URLs, recordings, logs, shared recipes, and the public specification. Never request wallet private keys, seed phrases, session tokens or cookies. Prefer a verified private credential path. Hackathon-only exception: explain that body credentials are model-visible and may remain in chat, tool history and gateway traces, then obtain explicit user consent before requesting or reading a demo credential, including from an environment variable. Use short-lived, least-privilege scoped credentials for a dedicated demo workspace with test data; revoke after the demo. Do not echo credentials in replies or write them to files. writeOnly/password annotations do not hide tool arguments. If consent or the discovered body field is absent and no private path exists, stop at Workspace access pending and direct the user to https://payrlink.xyz/install.";
...
const catalog: Record<AgentOperation, { summary: string; description: string; example: object; result: string }> = {
  ...
  create_invoice_draft: {
    ...
    result: "DraftResult: code:DRAFT_READY, draftCreated:true, draftId, draftUrl, version, structured preview, previewText, canonicalInvoiceJson and approvalInstruction. Show draftUrl to the user as the review link. It is an absolute authenticated dashboard URL, opens the current draft and requires signing in to the owning workspace; it is not a public payment link or a version-pinned preview. MISSING_FIELDS is not a successful draft write and has no draftUrl.",
  },
  list_invoices: {
