...
R07 is released as `v0.6.0` at `0d122231b0252f587a59868022b65dea83f6d525` ([PR #8](https://github.com/2manslkh/payr/pull/8)). It adds the Arc native-USDC settlement contract, guarded testnet EIP-712 signer, persisted authorization API, and crash-safe operator payment/recovery tooling. One separately approved `1 USDC` testnet invoice payment is verified, including exact event/balance facts and authorization-row read-back. This patch adds publication recovery fairness and stale-workspace sender-save protection. Reconciliation/browser-payment work developed separately is not included in this source snapshot; receipts, durable receipt delivery, and Claude MCP remain completion gates. See [R07 evidence and operation](docs/ops/r07-settlement.md) and the [review-fix release notes](docs/ops/review-fixes.md). Tags and merged PRs identify completed releases.
...
Local R06 evidence and its verification boundaries are in [`docs/ops/r06-documents.md`](docs/ops/r06-documents.md); R05 protocol evidence remains in [`docs/ops/r05-publication.md`](docs/ops/r05-publication.md). Released R07's [`operation record`](docs/ops/r07-settlement.md) establishes hosted document delivery, one approved initial invoice email, and a real operator payment. These are not receipt-email or deployed Claude proof. Browser payment/reconciliation developed outside this release needs its own integration evidence; receipts and durable delivery remain R08 work, and Claude MCP remains R09 work. The historical external-prerequisite ledger above retains its original observation date.
