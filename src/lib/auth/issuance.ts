...
import { IdentityError, type AuthNonce, type IdentityConfig, type IdentityRepository } from "../identity/contracts";
...
export async function admitNonceRequest(
  ...
  purpose: AuthNonce["purpose"] = "payr-login-v1",
): Promise<void> {
  ...
  const result = await repository.admitNonceIssuance({ walletHash: hash("wallet", `${purpose}:${wallet}:${ip}`), ipHash: hash("ip", ip) });
