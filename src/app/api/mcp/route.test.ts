...
import { POST as legacyPOST } from "./[token]/route";
...
it("blocks both direct MCP transports after explicit gateway cutover without opening the runtime", async () => {
  vi.stubEnv("PAYR_AGENT_GATEWAY_ONLY", "true");
  const header = await POST(request());
  const legacy = await legacyPOST(new Request(`https://example.test/api/mcp/${token}`, { method: "POST" }), { params: Promise.resolve({ token }) });
  for (const response of [header, legacy]) {
