...
export async function serveMcpRequest(request: Request, token: string) {
  if (process.env.PAYR_AGENT_GATEWAY_ONLY === "true") {
    return Response.json({ jsonrpc: "2.0", id: null, error: { code: -32000, message: "Use the Payr Bazantic gateway" } }, {
      status: 403, headers: { "Cache-Control": "private, no-store", "Referrer-Policy": "no-referrer", "X-Content-Type-Options": "nosniff", "X-Robots-Tag": "noindex, nofollow, noarchive" },
    });
  }
  try {
