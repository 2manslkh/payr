...
The public gateway MCP endpoint is ${PAYR_GATEWAY_MCP_URL}. Adding it enables tool discovery, not workspace access. Sign in through Privy and create a REST gateway connection in /app/connections for the operator-registered service. Prefer an operator-configured private credential path if available, separately from the gateway operator's service key. Private credential injection and live authenticated MCP-client execution remain unverified. The user confirmed Bazantic does not forward Authorization; Codex bearer/OAuth configuration is not this body-credential path, and client configuration does not automatically inject body credentials.

Approved hackathon exception: only with informed per-user consent, use a short-lived, least-privilege raw account credential for a dedicated demo workspace in generated gateway requestBody.accountCredential beside input (upstream top-level accountCredential beside input), without a Bearer prefix. Model/tool history and gateway traces may retain the secret; password/write-only annotations do not make it private. Never use service keys, wallet private keys, seed phrases, or browser cookies in this path. Keep secrets out of source control, public examples and recordings, and revoke the credential after the demo. Without a private path or this explicit consent, stop at "Workspace access pending". This exception changes guidance only, not authentication plumbing or action-approval requirements.

The source gateway catalog at /openapi/agent.json is not proof of the live imported catalog. Historical discovery evidence records that the currently imported gateway catalog lacks get_account_context; wallet discovery requires a catalog refresh, explicit wallet:read permission and the service operation scope. Verify access with the read-only get_account tool after configuring the private path or consenting to the demo exception, and ask the owner to confirm the returned workspace. Discovery alone does not verify workspace access. Never request the gateway service key or invent OAuth or header-forwarding support. Gateway account credentials cannot be used as direct MCP credentials.
...
For the gateway-first flow, see /install and /auth.md. The gateway at ${PAYR_GATEWAY_MCP_URL} exposes its own catalog; public discovery is not workspace authentication. The source /openapi/agent.json composes 12 operations, but historical discovery evidence records that the currently imported gateway catalog lacks get_account_context. It must be refreshed for wallet discovery and the current Publish & Send contract before activation. Prefer an operator-configured private credential path if available, with the separately scoped service key kept server-side. Optional get_account_context requires explicit wallet:read permission and the service operation scope, and never permits signing or spending. Confirm the actual tools through discovery, not the source operation count.

Approved hackathon exception: only with informed per-user consent, use a short-lived, least-privilege raw account credential for a dedicated demo workspace in generated gateway requestBody.accountCredential beside input (upstream top-level accountCredential beside input), without a Bearer prefix. Model/tool history and gateway traces may retain the secret; password/write-only annotations do not make it private. Never use service keys, wallet private keys, seed phrases, or browser cookies in this path. Keep secrets out of source control, public examples and recordings, and revoke the credential after the demo. Without a private path or this explicit consent, stop at "Workspace access pending". The user confirmed Bazantic does not forward Authorization; Codex bearer/OAuth configuration is not this body-credential path, and client configuration does not automatically inject body credentials. Verify access with the read-only get_account tool and have the owner confirm the returned workspace. Private injection and live authenticated MCP-client execution remain unverified; historical HTTP gateway evidence does not prove either. This exception changes guidance only, not authentication plumbing or action-approval requirements.
