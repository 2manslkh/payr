...
R09 adds a stateless Streamable HTTP endpoint at `/api/mcp/[token]` using the pinned official MCP SDK. Each request uses existing connector credential verification and atomic database token/IP admission. Protocol discovery consumes the existing read-only `invoice:status` action; tool calls consume their actual canonical action once. There is no fifth connector scope, session registry, SSE resume, or cookie authorization fallback. Unsupported methods and resume headers are rejected explicitly.

The four tools are `create_invoice_draft`, `publish_invoice`, `get_invoice_status`, and `void_invoice`. The portable workflow is `skills/payr-create-invoice/SKILL.md`. Tool calls delegate to released canonical services, including publication approval/version checks and F5 status projection.
...
3. Prove initialization and discovery return exactly the four tool names. Record client version and negotiated protocol version without copying credential-bearing requests.
4. Submit an incomplete draft request. Expect structured `MISSING_FIELDS`, `draftCreated: false`, and no invoice mutation. Supply confirmed synthetic client facts and an approved exact decimal amount. Create a complete draft and record its ID/version without protected links.
