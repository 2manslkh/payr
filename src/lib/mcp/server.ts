...
const tools: Tool[] = [
  ...
  { name: "create_invoice_draft", description: "Gather or revise a USDC invoice. Missing fields return MISSING_FIELDS with draftCreated:false and cause no mutation. On success, show the returned draftUrl as a review link: it opens the current draft in the authenticated owner dashboard, not a public payment page or version-pinned preview. Revision uses draftId plus expectedVersion on this same tool. Review the complete preview, applied defaults, and client-profile diff before explicit publication approval. No sender or payout authority." + common,
